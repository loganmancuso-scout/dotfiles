...
function init(){
  print_ascii_time "Starting init"
  ...
  sudo apt install -y vim git curl restic snap pipewire python3-full python3-pip unzip perl freerdp2-x11 libspa-0.2-bluetooth
  ...
  print_ascii_time "Completed init"
}
...
function install-frontend() {
  ...
  ./install.sh 
  popd
  # replace monitor config with custom setup
  echo "Update Monitors"
  sed -i '/monitor=,preferred,auto,1/c\monitor = eDP-1, 3000x2000@60,  3640x0, 1.6\nmonitor = DP-3,  2560x1080@144, 0x0,    1\nmonitor = DP-4,  1920x1080@75,  2560x-250, 1, transform, 3' $HOME/.config/hypr/UserConfigs/Monitors.conf
