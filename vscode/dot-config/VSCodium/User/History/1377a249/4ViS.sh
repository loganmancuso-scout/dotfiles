...
function helper() {
  ...
  for this_vault in "${vault_name[@]}"; do
    echo -e "START:\trestore $this_vault"
    local vault_path="$path_usb/$this_vault.vault"
    if [ -d "$vault_path" ]; then
      echo "Restic repository found $vault_path."
      # Define paths to restore for each vault
      sudo mkdir -p /tmp/restic-restore/$this_vault
      echo "sudo -E restic --verbose --repo "$vault_path" restore latest /tmp/restic-restore/$this_vault"
      echo -e "END:\trestore $this_vault"
    else
      echo "Restic repository Missing at $vault_path..."
      exit 1
    fi
