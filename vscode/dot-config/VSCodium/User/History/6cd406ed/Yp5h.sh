  ...
  for this_folder in "${raw_copy[@]}"; do
    ...
    case $this_folder in 
      "os-init")
        ...
        sudo cp -R "$HOME/.config/vivaldi" "$HOME/Documents/Backups/os-init/secrets/programs/vivaldi"
        sudo cp "$HOME/source/Personal/Infrastructure/Proxmox/global-secrets/env/terraform.tfvars" "$HOME/Documents/Backups/os-init/global_secrets.tfvars"
        sudo cp "$HOME/source/Personal/Infrastructure/Applications/vault/keys/keys.tar.gz" "$HOME/Documents/Backups/os-init/keys.tar.gz"
